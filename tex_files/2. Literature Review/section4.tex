...
OpenAlex~\cite{priem2022openalex} as the de facto open successor at scale,
linking 209\,M works to 213\,M disambiguated authors via 65k Wikidata-backed
concepts. There is no SPARQL endpoint, no reuse of SPAR ontologies, and the
JSON/REST API is the canonical interface. SemOpenAlex~\cite{farber2023semopenalex}
exists precisely to close that gap, re-lifting OpenAlex monthly into a
26-billion-triple RDF graph with 13 entity types, 87 relations, and released
embeddings on Zenodo. The MAKG lineage~\cite{farber2019makg,farber2022makgenhanced}
supplies the historical RDF anchor that SemOpenAlex inherits. A second cluster of property-graph systems comprises
S2AG~\cite{kinney2023semanticscholar} (which extends Ammar et al.'s literature
graph~\cite{ammar2018construction}), OAG~\cite{zhang2019oag,zhang2024oagbench},
and AceKG~\cite{wang2018acekg}. Two further systems take different design
priorities. ORKG~\cite{jaradeh2019orkg} provides curated semantic depth, and
OpenCitations~\cite{peroni2020opencitations,heibi2019coci,massari2024ocmeta,heibi2024ocindex}
models citations as first-class resources.
Table~\ref{tab:kg-infra} compares the eight systems on coverage, schema, query
interface, embeddings, and licence.
...
Embedding-based retrieval is most developed in Semantic Scholar, which layers
...
SciDocs. Scite~\cite{nicholson2021scite} sits closest to a text-plus-citation
hybrid, classifying citation statements as supporting, contrasting or
mentioning via a SciBERT classifier. A third-party
audit~\cite{bakker2023evaluating} reports F-measures between 0 and 0.58
against the vendor's claimed 80\%+ precision. The gap illustrates the broader
reliability problem with vendor-reported numbers in this cluster.
