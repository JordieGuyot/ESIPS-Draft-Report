...
publishes each citation link as its own addressable record with its
own identifier and metadata, rather than as a field on the citing
paper.  Table~\ref{tab:kg-infra} carries
...
Today's deployed search engines practise structured, text-centric, and
citation-graph retrieval separately, and no widely-used system seriously
combines them under a single ranking regime. Text-centric engines dominate daily use. Google
...
engine~\cite{gusenbauerhaddaway2020}.
% Comparator pass (July 2026): PubMed added because it is now an RQ3
% comparator (see research_design III-D6). Optional: cross-reference the
% II-A history subsection at the MEDLARS mention if its label is handy.
PubMed is the structured engine in the same daily-use tier. It descends from
MEDLARS and keeps that design, offering field-tagged Boolean search over
typed metadata and the MeSH controlled vocabulary, and the same comparison
that rules Google Scholar out rates PubMed as suitable for systematic
reviews~\cite{gusenbauerhaddaway2020}. Its coverage is confined to
biomedicine, and its matching is lexical, so a relevant paper worded
differently from the query can be missed. Among major open-access alternatives at
