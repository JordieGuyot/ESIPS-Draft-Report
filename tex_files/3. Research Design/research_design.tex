...
%
%  Notes:
%   * Figures use framed placeholders so this file compiles as-is.
%     Replace each with \includegraphics (commented lines show where).
%   * Cross-references to sections WITHIN this file use \ref to labels
%     defined here. References to other chapters (e.g. "Section II-B1")
%     and to the appendices are left as plain text for you to wire up.
%   * Table 1 (memory budget) had no data in the source, so a
%     placeholder table is provided.
%   * Source text is reproduced verbatim, including a few apparent typos
%     ("dumented", "nDGC") and explicit placeholders (<TBC>, [to be added]).
% =====================================================================
...
The design was derived from the knowledge graphs surveyed in Section~II-C1, which commonly share a similar sub-ontology over the six node types required for retrieval (Work, Person, Institution, Venue, Publisher, and Keyword) and their associated edges. This schema is based on that common structure.
...
For all purposes excluding benchmarks, the complete OpenAlex snapshot was used as the data source. As established in Section~II-C1, OpenAlex is the most suitable index due to its large corpus of works and pre-processed metadata.
...
\begin{table}[htbp]
    \centering
    \caption{The memory allocation budget}
    \label{tab:memory-budget}
    % TODO: populate with the memory allocation budget figures.
    \begin{tabular}{ll}
        \toprule
        \textbf{Component} & \textbf{Allocation} \\
        \midrule
        \multicolumn{2}{c}{\emph{Budget figures to be inserted.}} \\
        \bottomrule
    \end{tabular}
\end{table}

The resulting data was transformed to the structure of the ontology, and loaded into Memgraph using their dumented best practices. Memgraph was used as an in-memory graph database, on a virtual machine provisioned with sufficient memory to hold the full graph. Memgraph was chosen for the query performance of an in-memory store at this scale, for its MAGE library of graph algorithms, and for a licence compatible with the intended use. Because the graph resides in memory, the dense embeddings are quantised from 32-bit floating point to 8-bit integers to fit within the available memory, the budget for which is given in Table~\ref{tab:memory-budget}.
...
The normalised query is then converted into an Intermediate Representation (IR) using an LLM via a forced tool call whose schema mirrors the representation. The IR couples two components: firstly, a flat list of typed entities, each carrying a type, value, confidence and exact flag; secondly, a recursive boolean expression over those entities using AND, OR and NOT. This design aims to decouple LLM-based intent extraction from deterministic query construction, taking the flexibility of the former whilst keeping the correctness guarantee of the latter, as noted in Section~II-B3. The representation is then validated, dropping low-confidence entities and rejecting any query with no entities to anchor the traversal.
...
The benchmark was built by hand, because the literature (Section~\textless TBC\textgreater) found that no existing benchmark covers this task in the scholarly domain. It was constructed as follows:
...
The keyword generation module of Section~\ref{subsubsec:ingestion-storage} is evaluated against three keyphrase datasets, namely KPBiomed, OAGKX, and PubMedAKE, each of which pairs documents with reference keyphrases. Two F1 measures are reported: faithful, requiring a generated keyphrase to match a reference exactly; and semantic, in which an LLM judges whether a generated and a reference keyphrase are equivalent, which credits valid paraphrases that an exact match would reject. The candidate models benchmarked here are [to be added].

\subsubsection{Dense Retrieval}
\label{subsubsec:dense-retrieval}

The dense bi-encoder is evaluated on the ad-hoc search tasks of SciRepEval, namely Search, NFCorpus, and TREC-CoVID, measured by nDCG. SciRepEval is an established benchmark for scientific document retrieval (Section~II-D), and nDCG is the standard measure of ranking quality, which keeps the stage comparable to prior work. The candidates are SPECTER2 and text-embedding-3-small.

\subsubsection{Cross-Encoder Re-ranking}
\label{subsubsec:cross-encoder}

The cross-encoder is evaluated on the same SciRepEval ad-hoc search tasks under the same metric, by re-ranking the retrieved candidates, so the gain it adds can be read directly against the bi-encoder alone. The candidates include Cohere Rerank 4.0 Pro and self-hosted models such as mxbai-rerank-large-v1.
