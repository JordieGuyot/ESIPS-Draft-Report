...
%     Appendix B = worked example (query -> IR -> Cypher), being added;
%     Appendix C = prompts and tool schemas (keyword generation prompt,
%     IR-extraction system prompt + forced tool schema), to be added;
...
%     References to B and C are literal text until those appendices
%     exist, mirroring the Section~VI convention in the Introduction.
...
For all purposes excluding benchmarks, the complete OpenAlex snapshot of \textit{[FILL: release date]} was used as the data source, with the version pinned in line with the snapshot-specific citation practice argued for in Section~\ref{subsec:kg-infrastructure}. As established there, OpenAlex is the most suitable index due to its large corpus of works and pre-processed metadata.
% FILL (snapshot date): ingestion ran 23 Mar--1 Apr 2026 per the Gantt. The
% public releases immediately before that window were 2026-01-15 and
% 2026-02-03 per OpenAlex's RELEASE_NOTES (a March release may also exist).
% Definitive answer: `head RELEASE_NOTES.txt` at the root of the snapshot you
% downloaded, and put that date here.
...
For each work in the subset, a keyword generation module and an embedding generation module produce the per-work keywords and the dense embedding that the structured and dense retrieval stages depend on. The keyword module prompts a language model to extract keyphrases from the title and abstract, instructed to return only spans that occur in the source; generation runs at temperature zero with an output cap of 400 completion tokens, no reasoning effort requested, and all other sampling parameters at their API defaults, and the prompt is reproduced in Appendix~\ref{app:prompts}. Temperature zero does not make the hosted endpoint deterministic, which is why the module's run-to-run stability is measured separately (Section~\ref{subsec:evaluation-validation}).
% PENDING (the "10,000"): author confirms no candidate caps anywhere, so the
% 10,000 supplied against the 'keyphrase cap' slot is unattributed. A codebase
% grep for "10000" will settle what it is (or confirm it is stale). Also still
% open: does the prompt bound how many keyphrases a single work receives? The embedding module encodes the concatenated title and abstract into a single dense vector. A number of models for each module were benchmarked to inform the selection, as outlined in Section~\ref{subsec:evaluation-validation}, and the selected models are reported with those results.
...
% VERIFY (source precision + scheme): the author's "fp16 to int8" most likely
% described the WEIGHT quantisation of oversized benchmark models (see the
% evaluation-environment note in the Evaluation and Validation Design), not
% these stored vectors. If the deployed vectors come from the hosted
% bi-encoder's API, they are typically materialised as fp32 before the int8
% cast. Check the ingestion code's dtype at the cast and state "from fpXX";
% naming the conversion call also pins the scheme.
...
% the schema description; reconcile with the Appendix C tool schema when it
% lands (either the field remains and is unused for filtering, or remove it
% from the field list here).
...
% "10,000" from the fill answers remains unattributed --- see the note under
...
In the dense retrieval stage, the query is embedded at query time with the selected bi-encoder, and each candidate work carries the embedding precomputed for it at ingestion (Section~\ref{subsubsec:ingestion-storage}), stored at int8 in the graph. Similarity is cosine, computed over the candidate pool rather than through an approximate index, with the computation performed by Memgraph's built-in similarity function over the stored int8 vectors.
% VERIFY: name the exact Memgraph procedure/module used (one grep of the query
% code); that also settles how int8 is handled internally, which can otherwise
% be left to the database as an implementation detail. The 100 candidates most similar to the query are retained.

In the re-ranking stage, each retained candidate is paired with the query as a (query, title and abstract) input to the selected cross-encoder, and the single relevance score returned per pair determines the final ordering, with ties broken by citation count.
% VERIFY: confirm no input truncation is applied to long title-and-abstract
% pairs (or state the policy if one exists); author unsure. Not to be confused
% with the benchmark-side weight quantisation noted in the evaluation design.
