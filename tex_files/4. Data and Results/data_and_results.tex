...
This section reports the result of each component benchmark from the validation design in Section~\ref{subsec:evaluation-validation}, taking the components in turn.
...
Table~\ref{tab:ir-reasoning} isolates the effect of reasoning on the two GPT models in a separate single-run ablation, one run per condition, so its levels are not directly comparable to the 0.937 and 0.931 headlines of Table~\ref{tab:ir-accuracy}; the effect of interest is the within-model change from enabling reasoning. GPT-5.4-nano gains almost nothing from it, moving from 0.914 without reasoning to 0.922 with around 246 reasoning tokens, a difference of 0.008, and so reaches its accuracy in a single pass with no reasoning budget. GPT-5-mini behaves differently, scoring only 0.750 without reasoning and rising to 0.940 once reasoning is enabled, at a cost of around 896 reasoning tokens per query. Within the ablation, the stronger model is already at its ceiling unaided, whereas the weaker one reaches a comparable level only by spending a substantial token budget.
...
\begin{minipage}{\linewidth}
\footnotesize\itshape
Unlike the bi-encoder, a reranker stores no vectors, so its cost is latency, which scales with parameter count, rather than memory, which is why the stage runs over only around 100 candidates. Licences marked ``Open'' are self-hostable but their exact terms should be confirmed against the deployment before selection; the jina-reranker-v2 weights are CC-BY-NC-4.0 and so are non-commercial and not deployable in a commercial product.
\end{minipage}
\end{table}
...
% Placeholder. Results not yet available.
This evaluation compares the assembled system against a retrieval-augmented LLM and Google Scholar over a curated set of realistic queries, reported as a per-query characterisation of what each system retrieves alongside light relevance judgments over the returned results. It supplies the system-level evidence the component benchmarks cannot, and the basis for the RQ3 comparison in Section~\ref{subsec:rq3}. Results are reported in the final submission.