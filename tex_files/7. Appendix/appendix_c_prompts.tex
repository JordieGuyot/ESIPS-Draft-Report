...
keyphrases from the completion. The system prompt reproduced below is the deployed
\emph{generation\_tight} variant (\lstinline|prompt_variant: generation_tight|),
the procedure vendored from production and the one whose results the report body
describes. It is a mostly-extractive prompt that also asks for a few higher-level
keyphrases which need not appear verbatim, which is the behaviour the present and
absent scores of Section~\ref{sec:results-keywords} separate.

% NOTE (explicit, not silent): the source config.yaml sets
%   prompt_variant: generation_tight   (gen_version v2b), whose text lives in
% prompts.json -> "generation_system" and is the variant reproduced below. Two
% earlier variants exist in code (extraction | generation | generation_tight);
% the plain "extraction" prompt was the earlier deployed procedure and is NOT
% reproduced here, since the tabled runs (Tables IV-VI) and the report body all
% use generation_tight. The prompt asks for "up to 20" keyphrases: that is the
% count the MODEL is asked to produce. It is distinct from the retention/scoring
% cap of five (F1@5 scores the top five, and production retains at most five per
% work, Section III-C2). See the config block below and the note in III-D1.

\begin{lstlisting}
Generate a ranked list of up to 20 keyphrases for the scientific text below, for search and indexing, and return ONLY a JSON array of lowercase strings.

Write keyphrases the way a human indexer would: concise noun phrases, typically 1 to 3 words (4 at most). Do NOT output sentences, clauses, or long descriptive phrases.

List each distinct concept once, using its single most standard surface form. Do NOT include near-duplicate variants, abbreviation-and-expansion pairs, or reworded restatements of the same concept.

Preserve technical identifiers exactly as written, including any numbers (for example p53, il-6, brca1, covid-19).

Include both keyphrases that appear in the text and a few important higher-level keyphrases that capture the paper's topic, method, or domain but may not appear verbatim. Avoid generic filler such as study, research, or results.

Order the keyphrases from most to least central so that top-k truncation is meaningful.

Example output for a paper on p53 signalling in breast cancer:
["p53", "breast cancer", "tumor suppressor", "brca1", "apoptosis", "dna damage response", "oncogenesis"]
\end{lstlisting}
...
Output/parsing caps   : max_keywords (n) = 5; max_text_chars = 6000
\end{lstlisting}
